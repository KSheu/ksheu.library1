...
tensor_decomp = function(matrix, dims, method, xlabs = rownames(matrix), ylabs =colnames(matrix)[-1], zlabs = NULL, ranks = A@modes, savename="TEST"){
  ...
  if(method=="tucker"){
    ...
    U.1 = data.frame(tucker_decomp$U[[1]]);rownames(U.1) = make.unique(xlabs)
    U.2 = data.frame(tucker_decomp$U[[2]]);rownames(U.2) = ylabs
    U.3 = data.frame(tucker_decomp$U[[3]]);rownames(U.3) = zlabs
    
    pheatmap(U.1, cluster_cols = F, cluster_rows = F, scale="row")
    pheatmap(U.2, cluster_cols = F, cluster_rows = F, scale="none")
    pheatmap(U.3, cluster_cols = F, cluster_rows = F, scale="none")
    
    write.table(U.1, paste0(savename,"_U.1_",method,".txt"), quote=F,row.names = F, sep="\t")
    write.table(U.2, paste0(savename,"_U.2_",method,".txt"), quote=F,row.names = F, sep="\t")
    write.table(U.3, paste0(savename,"_U.3_",method,".txt"), quote=F,row.names = F, sep="\t")
  }
